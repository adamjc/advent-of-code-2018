...
fn main() -> Result<()> {
    ...
    let checksum = checksum(&exact_digits_count);
...
fn checksum(vec: &Vec<(i32, i32)>) -> i32 {
  let exact_digits_count = vec.into_iter()
  ...
  exact_digits_count[0] * exact_digits_count[1]
}
...
fn find_exacts (id: &str) -> (i32, i32) {
  ...
  println!("{:?} contains {:?} twice and {:?} thrice", id, twice, thrice);
